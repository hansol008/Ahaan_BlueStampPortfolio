...
## Summary
For my modifications, my goal was to add a status LED to indicate whether the robot can see the ball or not, and to create a real-time map of the robot's movement. First, I had to add an RGB LED to my breadboard to accommodate the status LED. In an RGB LED, there are separate connection points for each color. The blue and green connection points take 3.3V of power, while the red takes 2.2V. To avoid burning my LED, I added voltage dividers for all connections. I had a 165-ohm resistor each for the green and blue, but the red needed more. For this, I used a 22 Ohm resistor, a 75 Ohm resistor, and a 110 Ohm resistor. I found these values by solving for the output voltage knowing that 5V came in, and I had to output 0V. After setting up the circut for the RGB LED, I had to connect each color to a GPIO pin. To get the LED to work, I had to set up a method that turns on all the colors at the same time, which makes the LED shine white light. By changing which colors turn on, you can change the color of the LED.

For my second modification, I decided to create a real-time map of the robot's movement. I originally wanted to use an MPU6050, which is a type of IMU, or inertial measurement unit. The MPU6050 has a gyroscope and accelerometer, which is what I was planning to use for my mapping feature. After coding test code for the MPU6050, I kept having a problem that threw off the angle reading of the MPU. After this problem, I decided to switch to using two optical rotary encoders to accurately display the movement of the robot. An optical rotary encoder is a sensor that converts rotation into electrical signals using light. It tracks the position, speed, and direction of a rotating encoder disk. The encoder disk is a wheel with periodic holes, which periodically break the light's connection, counting it as one tick. Using the ticks and which motors are moving, the optical rotatory encoders can map the movement of the robot based on the motor's movement. To make sure the optical rotary encoder can accurately replicate the real-life distance the robot moves, I had to input the circumference of the encoder disk and the distance between the wheels so that the optical rotary encoder can accurately convert the rotations to centimeters. With the information from the optical rotary encoder, I used turtle, a Python drawing library, to draw the map in a new map window. Also, with the distance given by the optical rotary encoders, I calculated the total distance travelled by the robot and displayed that on the map window.

## Challenges
The main challenge I had with these modifications was with the mapping feature. I originally wanted to use just the code by having the turtle move based on how the motors moved in the code. This did not work because when the motors moved in spurts, the turtle counted it as full movements, and any delays I used were not registered by the turtle. After this, I decided to try an MPU6050 for directions. The MPU6050 was giving slight angle read errors, and as the robot kept turning, the errors would pile up and offset the actual direction the robot was facing. Using calculus, you were supposed to use integrals to find the exact angle since an area equation would either overshoot or undershoot the angle. Before switching to the optical rotary encoder, I decided to overshoot the angle, which added an extra 0.001 degrees every time the direction changed and was updated. After a couple turns, the original starting point was 20-30 degrees to the right of its starting position of 90 degrees. I overcame this challenge by finding another way to map the movement of the robot, by using optical rotary encoders.

Another challenge I faced was calculating the resistance values for the voltage dividers for the RGB LED. I kept using the incorrect amplitude. I assumed my ampage was 0.05A when it was closer to 0.03A. This made me miscalculate the values for the resistors and restrict current even more, lowering the brightness of the LED. I was able to fix this by using the multimeter to read the amperage I was getting from the Raspberry Pi and plug that into my equation and solve.
...
# Code
Here is the final code that was used during demo night. All pins are as displayed on the schmeatic above.
<pre style="background:#fdfdfd; border:none; height:40pc">
